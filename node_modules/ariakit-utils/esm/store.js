...
import { useInitialValue, useWrapElement, useLazyValue, useSafeLayoutEffect } from './hooks.js';
...
function useStoreProvider(_ref, context) {
  ...
  return useWrapElement(props, element => {
...
function useStorePublisher(state) {
  const listeners = useLazyValue(() => new Set());
  useSafeLayoutEffect(() => {
    patchState(state);
