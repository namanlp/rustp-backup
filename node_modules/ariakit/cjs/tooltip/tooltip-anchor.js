...
const useTooltipAnchor = system.createHook(_ref => {
  ...
  const onFocus = hooks.useEvent(event => {
    ...
    if (event.defaultPrevented) return; // When using multiple anchors for the same tooltip, we need to re-render
  ...
  const onMouseEnter = hooks.useEvent(event => {
  ...
  const onMouseLeave = hooks.useEvent(event => {
