...
function canProxyKeyboardEvent(event, state) {
  ...
  if (dom.isTextField(target)) {
    ...
    const focusingInputOnly = state.activeId === null; // Pressing Home or End keys on the text field should only be allowed when
    ...
    const isHomeOrEnd = event.key === "Home" || event.key === "End"; // If there are no rows or the input is the only focused element, then we
...
function useKeyboardEventProxy(state, activeItem, onKeyboardEvent, previousElementRef) {
  return hooks.useEvent(event => {
    ...
    const previousElement = previousElementRef == null ? void 0 : previousElementRef.current; // If the active item element is not the previous element, this means that
...
function useScheduleFocus(activeItem) {
  ...
  react.useEffect(() => {
    ...
    if (scheduled && activeElement) {
      setScheduled(false);
      focus.focusIntoView(activeElement);
    }
...
const useComposite = system.createHook(_ref => {
  ...
  const activeItem = react.useMemo(() => __utils.findEnabledItemById(state.items, state.activeId), [state.items, state.activeId]);
  ...
  hooks.useSafeLayoutEffect(() => {
    ...
    scheduleFocus();
  }, [composite, focusOnMove, state.moves]); // If composite.move(null) has been called, the composite container (this
  ...
  hooks.useSafeLayoutEffect(() => {
    ...
    if (!element) return;
    const previousElement = previousElementRef.current; // We have to clean up the previous element ref so an additional blur
    ...
    if (focus.hasFocus(element)) {
      events.fireFocusEvent(element, {
        relatedTarget: previousElement
      });
    } else {
      element.focus();
    }
  }, [composite, state.moves]); // At this point, if the activeId has changed and we still have a
  ...
  hooks.useSafeLayoutEffect(() => {
    ...
    const activeElement = (_activeItemRef$curren2 = activeItemRef.current) == null ? void 0 : _activeItemRef$curren2.ref.current;
    const relatedTarget = activeElement || dom.getActiveElement(previousElement);
    events.fireBlurEvent(previousElement, {
      relatedTarget
    });
  }, [virtualFocus, composite, state.activeId]);
  const onKeyDownCapture = useKeyboardEventProxy(state, activeItem, props.onKeyDownCapture, previousElementRef);
  const onKeyUpCapture = useKeyboardEventProxy(state, activeItem, props.onKeyUpCapture, previousElementRef);
  const onFocusCaptureProp = props.onFocusCapture;
  const onFocusCapture = hooks.useEvent(event => {
    onFocusCaptureProp == null ? void 0 : onFocusCaptureProp(event);
    if (event.defaultPrevented) return;
    if (!virtualFocus) return;
    const previousActiveElement = event.relatedTarget;
    const isSilentlyFocused = __utils.silentlyFocused(event.currentTarget);

    if (events.isSelfTarget(event) && isSilentlyFocused) {
      ...
      event.stopPropagation(); // We keep track of the previous active item element so we can manually
  ...
  const onFocus = hooks.useEvent(event => {
    ...
    if (virtualFocus) {
      ...
      if (events.isSelfTarget(event) && !__utils.isItem(state.items, event.relatedTarget)) {
        ...
        misc.queueMicrotask(scheduleFocus);
      }
  ...
  const onBlurCapture = hooks.useEvent(event => {
    ...
    const nextActiveElementIsItem = __utils.isItem(state.items, nextActiveElement);
    const previousElement = previousElementRef.current;
    previousElementRef.current = null; // This is an intermediate blur event: blurring the composite container
    ...
    if (events.isSelfTarget(event) && nextActiveElementIsItem) {
      ...
      const targetIsItem = __utils.isItem(state.items, event.target); // If target is not a composite item, it may be the composite element
