...
function useAutoFocusOnShow(_ref) {
  ...
  const mountedRef = hooks.useLiveRef(state.mounted); // Resets autoFocusOnShow
...
function useAutoFocusOnHide(_ref2) {
  ...
  const onFocusProp = hooks.useEvent(props.onFocus);
...
const useHovercard = system.createHook(_ref3 => {
  ...
  const hideTimeoutRef = react.useRef(0);
  const enterPointRef = react.useRef(null);
  const {
    portalRef,
    domReady
  } = hooks.usePortalRef(portal, props.portalRef);
  const hideOnEscapeProp = hooks.useBooleanEvent(hideOnEscape);
  const hideOnControlProp = hooks.useBooleanEvent(hideOnControl); // Hide on Escape/Control. Popover already handles this, but only when the
  ...
  const hideOnHoverOutsideProp = hooks.useBooleanEvent(hideOnHoverOutside);
  const mayDisablePointerEvents = !!disablePointerEventsOnApproach;
  const disablePointerEventsProp = hooks.useBooleanEvent(disablePointerEventsOnApproach); // Checks whether the mouse is moving toward the hovercard. If not, hide the
