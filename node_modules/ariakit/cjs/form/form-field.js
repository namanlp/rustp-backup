...
const useFormField = system.createHook(_ref => {
  ...
  const name = "" + nameProp;
  state = store.useStore(state || __utils.FormContext, ["setFieldTouched", "useValidate", "setError", react.useCallback(s => s.getError(name), [name]), react.useCallback(s => s.getFieldTouched(name).toString(), [name]), react.useCallback(s => findItem(s.items, name, "label"), [name]), react.useCallback(s => findItem(s.items, name, "error"), [name]), react.useCallback(s => findItem(s.items, name, "description"), [name])]);
  const ref = react.useRef(null);
  const id = hooks.useId(props.id);
  (_state = state) == null ? void 0 : _state.useValidate(async () => {
    const element = getNamedElement(ref, name);
    if (!element) return; // Flush microtasks to make sure the validity state is up to date
  ...
  const touchOnBlurProp = hooks.useBooleanEvent(touchOnBlur);
  ...
  const invalid = !!((_state4 = state) != null && _state4.getError(name)) && state.getFieldTouched(name);
  props = {
    id,
