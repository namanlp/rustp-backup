...
const useSelect = system.createHook(_ref => {
  ...
  const moveOnKeyDownProp = hooks.useBooleanEvent(moveOnKeyDown);
  const toggleOnPressProp = hooks.useBooleanEvent(toggleOnPress);
  ...
  props = hooks.useWrapElement(props, element => /*#__PURE__*/jsxRuntime.jsxs(jsxRuntime.Fragment, {
    children: [/*#__PURE__*/jsxRuntime.jsx(visuallyHidden_visuallyHidden.VisuallyHidden, {
      ...
      required: required,
      value: state.value,
      multiple: multiSelectable // Even though this element is visually hidden and is not tabbable,
      ...
      ,
      onFocus: () => {
        var _state$selectRef$curr;

        return (_state$selectRef$curr = state.selectRef.current) == null ? void 0 : _state$selectRef$curr.focus();
      },
      onChange: event => {
  ...
  }), [label, labelledBy, name, form, required, state.value, multiSelectable, state.selectRef, state.setValue, values]);
