...
function useComboboxState(_temp) {
  ...
  const prevActiveId = hooks.usePreviousValue(composite.activeId);
  const prevMoves = hooks.usePreviousValue(composite.moves);
  ...
  const activeValue = react.useMemo(() => {
    var _composite$items$find;

    if (!moved) return undefined;
    return (_composite$items$find = composite.items.find(item => item.id === composite.activeId && item.value)) == null ? void 0 : _composite$items$find.value;
  ...
  hooks.useUpdateLayoutEffect(() => {
    if (popover.open) return; // We need to reset the composite state when the popover is closed.
