...
const useCombobox = system.createHook(_ref => {
  ...
  const valueChangedRef = react.useRef(false); // We can only allow auto select when the combobox focus is handled via the
  ...
  const [canInline, setCanInline] = react.useState(inline); // If the inline autocomplete is enabled in a update, we need to update the
  ...
  const showOnChangeProp = hooks.useBooleanEvent(showOnChange);
  const setValueOnChangeProp = hooks.useBooleanEvent(setValueOnChange);
  const onChange = hooks.useEvent(event => {
  ...
  const onCompositionEnd = hooks.useEvent(event => {
  ...
  const showOnMouseDownProp = hooks.useBooleanEvent(showOnMouseDown);
  const onMouseDown = hooks.useEvent(event => {
  ...
  const showOnKeyDownProp = hooks.useBooleanEvent(showOnKeyDown);
  ...
  const onBlur = hooks.useEvent(event => {
