...
const isSafariBrowser = platform.isSafari();
...
const useFocusable = system.createHook(_ref => {
  ...
  const onMouseDown = hooks.useEvent(event => {
    ...
    if (!dom.isButton(element) && !isNativeCheckboxOrRadio(element)) return; // In future versions os Safari, it may change this behavior and start
    ...
    events.queueBeforeEvent(element, "mouseup", () => {
      element.removeEventListener("focusin", onFocus, true);
      if (receivedFocus) return;
      focus.focusIfNeeded(element);
    });
  });
  ...
  const handleFocusVisible = (event, currentTarget) => {
    ...
    if (!focus.hasFocus(element)) return;
    setFocusVisible(true);
  };

  const onKeyDownCaptureProp = props.onKeyDownCapture;
  const onKeyDownCapture = hooks.useEvent(event => {
    ...
    const element = event.currentTarget;
    misc.queueMicrotask(() => handleFocusVisible(event, element));
  });
  ...
  const onFocusCapture = hooks.useEvent(event => {
    ...
    if (isKeyboardModality || isAlwaysFocusVisible(event.target)) {
      misc.queueMicrotask(applyFocusVisible);
    } // See https://github.com/ariakit/ariakit/issues/1257
    else if (isAlwaysFocusVisibleDelayed(event.target)) {
      events.queueBeforeEvent(event.target, "focusout", applyFocusVisible);
    } else {
      setFocusVisible(false);
    }
  });
