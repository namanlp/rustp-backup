...
function isNativeClick(event) {
  ...
  return dom.isButton(element) || element.tagName === "SUMMARY" || element.tagName === "INPUT" || element.tagName === "TEXTAREA" || element.tagName === "A" || element.tagName === "SELECT";
}
/**
      ...
      if (isEnter) {
        if (!nativeClick) {
          ...
          const {
            view,
            ...eventInit
          } = event; // Fire a click event instead of calling element.click() directly
          // so we can pass along the modifier state.

          const click = () => events.fireClickEvent(element, eventInit); // If this element is a link with target="_blank", Firefox will
          ...
          if (platform.isFirefox()) {
            events.queueBeforeEvent(element, "keyup", click);
          } else {
            misc.queueMicrotask(click);
          }
        }
      } else if (isSpace) {
  ...
  const onKeyUp = hooks.useEvent(event => {
    ...
    if (activeRef.current && isSpace) {
      ...
      if (!isNativeClick(event)) {
        ...
        misc.queueMicrotask(() => events.fireClickEvent(element, eventInit));
      }
    }
  });
  props = {
