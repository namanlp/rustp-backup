...
const useMenuButton = system.createHook(_ref => {
  ...
  const parentMenu = store.useStore(__utils.MenuContext, ["items", "move", "contentElement"]);
  ...
  const onFocus = hooks.useEvent(event => {
    ...
    state.setActiveId(null); // When the menu button is focused, we'll only show its menu if it's in a
    // menu bar
  ...
  const onClick = hooks.useEvent(event => {
    ...
    if (event.defaultPrevented) return;
    const isKeyboardClick = !event.detail; // When the menu button is clicked, if the menu is hidden or if it's a
  ...
  const id = hooks.useId(props.id); // When the menu button is rendered inside another menu, we set the role
  // attribute here so it doesn't get overridden by the button component with
  // role="button".

  const role = hasParentMenu || parentIsMenuBar ? dom.getPopupItemRole(parentMenu == null ? void 0 : parentMenu.contentElement, "menuitem") : undefined;
  props = {
    id,
    role,
