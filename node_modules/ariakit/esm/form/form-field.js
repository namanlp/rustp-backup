...
import { useId, useBooleanEvent, useEvent, useForkRef } from 'ariakit-utils/hooks';
...
const useFormField = createHook(_ref => {
  ...
  const name = "" + nameProp;
  state = useStore(state || FormContext, ["setFieldTouched", "useValidate", "setError", useCallback(s => s.getError(name), [name]), useCallback(s => s.getFieldTouched(name).toString(), [name]), useCallback(s => findItem(s.items, name, "label"), [name]), useCallback(s => findItem(s.items, name, "error"), [name]), useCallback(s => findItem(s.items, name, "description"), [name])]);
