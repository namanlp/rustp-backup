...
import { useInitialValue, useControlledState, useLazyValue, useSafeLayoutEffect, useLiveRef } from 'ariakit-utils/hooks';
...
function useFormState(props) {
  ...
  const submitCallbacks = useLazyValue(() => new Set());
  const validateCallbacks = useLazyValue(() => new Set());
  ...
  const validate = useCallback(async () => {
    ...
    try {
      ...
      const results = callbacks.map(callback => callback()); // Wait for the next frame to allow the errors to be set on the state.
