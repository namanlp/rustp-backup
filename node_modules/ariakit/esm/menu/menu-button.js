...
import { useEvent, useId, useForkRef } from 'ariakit-utils/hooks';
...
const useMenuButton = createHook(_ref => {
  ...
  const parentMenu = useStore(MenuContext, ["items", "move", "contentElement"]);
  ...
  const id = useId(props.id); // When the menu button is rendered inside another menu, we set the role
  // attribute here so it doesn't get overridden by the button component with
  // role="button".

  const role = hasParentMenu || parentIsMenuBar ? getPopupItemRole(parentMenu == null ? void 0 : parentMenu.contentElement, "menuitem") : undefined;
  props = {
    id,
    role,
