...
import { useId, useEvent, useBooleanEvent, useWrapElement, useForkRef, useSafeLayoutEffect } from 'ariakit-utils/hooks';
...
import { C as CompositeContext, j as CompositeRowContext, k as getContextId, i as isItem, l as focusSilently, m as CompositeItemContext, f as findEnabledItemById } from '../__utils-7009759c.js';
...
const useCompositeItem = createHook(_ref => {
  ...
  const onFocus = useEvent(event => {
    ...
    if (!isSelfTarget(event)) return; // and the composite item is not a text field or contenteditable element.
    ...
    const fromComposite = event.relatedTarget === composite || isItem(state.items, event.relatedTarget);
